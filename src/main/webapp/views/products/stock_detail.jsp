...
<body id="kt_body"
    ...
    function updateStockDetailTable(data) {
    	var formattedUnitQuantity = viewNum(data[0].unitQuantity);
    	var formattedPurchasePrice = viewNum(data[0].purchasePrice);
        ...
        document.getElementById('unitQuantity').textContent = formattedUnitQuantity;
        document.getElementById('purchasePrice').textContent = formattedPurchasePrice;
    }
...
function viewNum(Num) {
    var formattedNum = Num.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
    return formattedNum;
}
</script>
